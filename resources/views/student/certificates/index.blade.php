        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="mb-8">
                ...
                <p class="mt-2 text-slate-500">
                    View earned verified certificates for Courses & Industry Projects with unique credential IDs.
                </p>
            </div>
            ...
            <div class="flex border-b border-gray-200 mb-8 gap-2">
                ...
                <button type="button"
                    ...
                    <span class="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700 font-extrabold">{{ $projects->count() }}</span>
                </button>
            </div>
            ...
            <div id="tab-courses" class="tab-content">
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    @forelse($courses as $course)
                        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4 hover:shadow-md transition">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <h2 class="text-lg font-bold text-slate-900 leading-snug">{{ $course->name }}</h2>
                                </div>
                            ...
                            <div class="space-y-2 text-xs">
                                <div class="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                    <p class="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Instructor</p>
                                    <p class="mt-0.5 font-bold text-slate-800">{{ $course->user->name ?? 'Lecturer' }}</p>
                                </div>

                                <div class="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                    <p class="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Status</p>
                                ...
                                @if($course->verified_final_attempt)
                                    <div class="rounded-xl border border-slate-100 bg-slate-50 p-3 flex justify-between items-center">
                                        <p class="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Verified Score</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    @forelse($projects as $project)
                        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4 hover:shadow-md transition">
                            ...
                            <div class="space-y-2 text-xs">
                                <div class="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                    <p class="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Author / Vendor</p>
                                    <p class="mt-0.5 font-bold text-slate-800">{{ $project->user->name ?? 'Vendor' }}</p>
                                </div>

                                <div class="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                    <p class="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Difficulty Level</p>
                                    <p class="mt-0.5 font-bold text-indigo-600">{{ ucfirst($project->difficulty_level) }}</p>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="col-span-full rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center text-slate-500 shadow-sm">
                            Belum ada project yang diselesaikan. Join project di menu Projects untuk mendapatkan sertifikat project.
                        </div>
                    @endforelse
                </div>
            </div>
