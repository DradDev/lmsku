            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8">
                <div class="mb-4 flex flex-wrap gap-2">
                    @if($result->is_verified)
                        <span class="rounded-full bg-emerald-100 border border-emerald-200 px-3 py-1 text-xs font-bold text-emerald-800 flex items-center gap-1">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
                            Graded
                        </span>
                    @else
                        <span class="rounded-full bg-amber-100 border border-amber-200 px-3 py-1 text-xs font-bold text-amber-800 flex items-center gap-1">
                    ...
                    @if(!empty($result->blockchain_hash))
                        <span class="rounded-full bg-indigo-100 border border-indigo-200 px-3 py-1 text-xs font-bold text-indigo-800 flex items-center gap-1">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div>
                        <span class="text-xs font-bold text-indigo-600 uppercase tracking-wider">Quiz Evaluation Result</span>
                        <h1 class="text-2xl font-black text-slate-900 mt-1">
                            {{ $result->quiz->title ?? 'Quiz Result' }}
                        </h1>
                        <p class="mt-2 text-sm text-slate-600">
                            Course: <span class="font-bold text-slate-800">{{ $result->quiz->course->name ?? '-' }}</span>
                        </p>
                        <p class="mt-1 text-xs text-slate-400">
                            Completion Date: <span class="font-medium text-slate-600">{{ $result->created_at?->format('d M Y H:i') ?? '-' }}</span>
                        </p>
                    </div>

                    <div class="bg-gradient-to-tr from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-5 text-center min-w-[150px] shadow-sm">
                        <p class="text-xs uppercase font-extrabold tracking-widest text-indigo-600">Final Score</p>
                        <h2 class="mt-1 text-4xl font-black text-indigo-700">{{ $result->score }}</h2>
                        <span class="text-[11px] font-semibold text-slate-500">Competency Score</span>
                    </div>
                </div>
