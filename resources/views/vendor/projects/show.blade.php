            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 space-y-4">
                        ...
                        <!-- Main Skill Card -->
                        <div class="p-3.5 bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 rounded-xl space-y-1.5">
                            <span class="text-[10px] font-extrabold uppercase tracking-widest text-purple-600 block">Primary Skill Requirement</span>
                    ...
                    <div class="bg-gradient-to-br from-slate-900 via-purple-950 to-indigo-950 rounded-2xl p-5 shadow-md text-white space-y-4 border border-purple-800/50">
                        <div class="flex items-center justify-between border-b border-purple-800/60 pb-3">
                            ...
                            <a href="{{ route('vendor.projects.talent-pool', $project) }}"
                               class="text-[11px] font-bold text-purple-300 hover:text-white underline">
                                Lihat Semua →
                            </a>
                        </div>

                        @if(!isset($recommendedStudents) || $recommendedStudents->isEmpty())
                            <p class="text-xs text-purple-200 italic">Belum ada mahasiswa yang memenuhi kualifikasi talent pool.</p>
                        @else
                            <div class="space-y-3">
                                @foreach($recommendedStudents as $index => $recStudent)
                                    @php
                                        $mScore = $recStudent->match_score;
                                        $scoreColor = $mScore >= 80 ? 'bg-emerald-500 text-white' : ($mScore >= 60 ? 'bg-purple-400 text-white' : 'bg-amber-400 text-amber-950');
                                    @endphp
                                    ...
                                    <div class="bg-white/10 backdrop-blur-md border border-white/15 rounded-xl p-3 space-y-2.5">
                                        <div class="flex items-center justify-between gap-2">
                                            <div class="flex items-center gap-2">
                                                ...
                                                @else
                                                    <div class="w-8 h-8 rounded-full bg-purple-600 text-white flex items-center justify-center font-black text-xs border border-white/20">
                                                        {{ strtoupper(substr($recStudent->name, 0, 1)) }}
                                                    </div>
                                                @endif
                                                <div class="overflow-hidden">
                                                    <h5 class="font-bold text-xs text-white truncate max-w-[110px]">{{ $recStudent->name }}</h5>
                                                    <span class="text-[10px] text-purple-200 font-medium block truncate max-w-[110px]">{{ $recStudent->peminatan ?? 'General' }}</span>
                                                </div>
                                            </div>
