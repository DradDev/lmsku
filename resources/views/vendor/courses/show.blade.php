                            ...
                            @forelse ($quizzes as $quiz)
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button" onclick="document.getElementById('edit-quiz-form-vendor-{{ $quiz->id }}').classList.toggle('hidden')" class="rounded-xl bg-amber-50 border border-amber-200 text-amber-800 px-3.5 py-2 text-xs font-bold hover:bg-amber-100 transition shadow-xs">
                                            Waktu & Durasi
                                        </button>

                                        <a href="{{ route('vendor.quizzes.show', $quiz) }}"
                                            class="rounded-xl bg-purple-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-purple-700 shadow-xs">
                                ...
                                <div id="edit-quiz-form-vendor-{{ $quiz->id }}" class="hidden mt-4 pt-4 border-t border-slate-200">
                                    <form method="POST" action="{{ route('vendor.courses.quizzes.update', [$course->id, $quiz->id]) }}" class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs bg-amber-50/40 p-4 rounded-xl border border-amber-200">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                <input type="number" id="edit_vendor_attempts_{{ $quiz->id }}" name="max_attempts" value="{{ old('max_attempts', $quiz->max_attempts) }}" min="0" max="100" class="w-full rounded-xl border-slate-300 p-2 text-xs font-semibold">
                                                <label class="inline-flex items-center gap-1.5 px-2.5 py-2 bg-white border border-slate-300 rounded-xl cursor-pointer hover:bg-slate-50 transition whitespace-nowrap">
                                                    <input type="checkbox" name="is_unlimited" value="1" @checked($quiz->max_attempts === 0) onchange="document.getElementById('edit_vendor_attempts_{{ $quiz->id }}').disabled = this.checked; if(this.checked){ document.getElementById('edit_vendor_attempts_{{ $quiz->id }}').value = 0; }" class="rounded text-purple-600 focus:ring-purple-500">
                                                    <span class="text-[11px] font-bold text-slate-700">Unlimited</span>
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="md:col-span-2 flex justify-end gap-2 pt-2 border-t border-amber-200">
                                            <button type="button" onclick="document.getElementById('edit-quiz-form-vendor-{{ $quiz->id }}').classList.add('hidden')" class="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-lg">Batal</button>
