<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-2xl shadow-md border border-slate-800 p-6 text-white">
                <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                    <div>
                        <div class="flex items-center gap-2 text-xs font-bold text-indigo-400 uppercase tracking-widest mb-1">
                            <span>COMPRO Talent Screening Engine</span>
                            <span class="px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 text-[10px]">Active Project</span>
                        </div>
                        <h3 class="text-2xl font-black text-white">
                            {{ $project->title }}
                        </h3>
                        <p class="text-xs text-slate-300 mt-1 max-w-3xl leading-relaxed">
                            Screening otomatis kompetensi mahasiswa berdasarkan bobot Kompetensi Quiz (50%), Peminatan/Spesialisasi (30%), dan Track Record Portofolio (20%).
                        </p>
                    </div>

                    <div class="flex items-center gap-2 shrink-0">
                        <a href="{{ route('lecturer.projects.show', $project) }}"
                           class="inline-flex items-center gap-1.5 px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold transition border border-white/15">
                ...
                <div class="mt-5 pt-4 border-t border-slate-800 flex flex-wrap gap-2 text-xs">
                    <span class="px-3 py-1 bg-indigo-500/20 text-indigo-200 border border-indigo-500/30 rounded-lg font-bold">
                        Difficulty: {{ ucfirst($project->difficulty_level) }}
                    </span>
                    <span class="px-3 py-1 bg-slate-800 text-slate-300 rounded-lg font-medium">
                        Estimasi: {{ $project->duration_days }} Hari
                    </span>
                    <span class="px-3 py-1 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-lg font-bold">
                        Kuota: {{ $project->participations()->count() }} / {{ $project->max_students }} Pendaftar
                    </span>

                    @foreach($project->skills as $skill)
                    <span class="px-3 py-1 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded-lg font-bold">
                        Requirement: {{ $skill->name }} {{ $skill->pivot->is_main ? '(Main)' : '' }}
                    </span>
                    @endforeach
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-5">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-4">
                    ...
                    <span class="px-3 py-1 bg-indigo-50 text-indigo-800 border border-indigo-100 rounded-full text-xs font-black self-start md:self-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Cari Nama / Peminatan</label>
                        <div class="relative">
                            <input type="text" id="talent-search-input" onkeyup="filterTalents()" placeholder="Ketik nama mahasiswa..."
                                   class="w-full pl-9 pr-3 py-2 text-xs font-semibold rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Status Undangan</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Tingkat Match Score</label>
                ...
                <div class="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                    <p class="text-gray-500 text-sm italic">Belum ada data talenta mahasiswa di sistem.</p>
                </div>
                @else
                <div id="talent-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pt-2">
                    ...
                    @php
                        ...
                        $scoreBadgeClass = $matchScore >= 80 ? 'bg-emerald-500 text-white' : ($matchScore >= 60 ? 'bg-indigo-600 text-white' : 'bg-amber-500 text-white');
                    ...
                    <div class="talent-card border border-gray-200 rounded-2xl p-5 hover:shadow-lg hover:border-indigo-300 transition-all flex flex-col justify-between relative bg-white"
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3.5 mb-4">
                                @if($student->avatar_url)
                                    <img src="{{ $student->avatar_url }}" alt="" class="w-12 h-12 rounded-full object-cover border-2 border-indigo-100 shadow-sm" onerror="this.style.display='none';">
                                @else
                                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 text-white flex items-center justify-center font-black text-lg shadow-sm border border-indigo-100">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 font-medium">
                                        Fokus: <span class="font-bold text-gray-700">{{ $student->peminatan ?? 'General Track' }}</span>
                                    </p>
                                </div>
                            </div>

                            <!-- Match Score Progress Bar -->
                            <div class="mb-4 bg-gray-50 p-2.5 rounded-xl border border-gray-100 space-y-1.5">
                                <div class="flex justify-between text-[11px] font-bold text-gray-600">
                                    <span>Skor Kualifikasi Competency</span>
                                    <span class="text-indigo-600 font-extrabold">{{ $matchScore }}%</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                    <div class="h-2 rounded-full {{ $matchScore >= 80 ? 'bg-emerald-500' : ($matchScore >= 60 ? 'bg-indigo-600' : 'bg-amber-500') }}"
                            ...
                            <div class="space-y-2 mb-4">
                                <div class="text-xs text-gray-600">
                                    <span class="font-bold text-gray-800">Kompetensi Utama (Quiz):</span>
                                    <div class="flex flex-wrap gap-1 mt-1">
                                        @forelse($student->skillProfiles->take(3) as $sp)
                                        <span class="px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-lg text-[11px] font-medium">
                                            {{ $sp->skill->name ?? 'Skill' }} ({{ round($sp->avg_score) }})
                                        </span>
                                        @empty
                                        <span class="text-gray-400 italic text-[11px]">Belum ada hasil kuis</span>
                                        @endforelse
                                    </div>
                                </div>

                                <div class="text-xs text-gray-600 flex items-center justify-between pt-1">
                                    <span class="font-bold text-gray-800">Project Selesai:</span>
                                    <span class="font-black text-indigo-700">{{ $student->completedProjects->count() }} Karya Disetujui</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-3 border-t border-gray-100 flex items-center gap-2">
                            ...
                            @else
                                <form action="{{ route('lecturer.projects.invite', [$project, $student]) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="py-2 px-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition shadow-sm">
                                        + Invite Talent
                                    </button>
                                </form>
                            @endif
