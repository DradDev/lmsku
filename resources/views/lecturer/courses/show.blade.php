<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-50 py-10">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <!-- PERMINTAAN RETAKE KUIS (SCALABLE 50+ MAHASISWA) -->
            @if(isset($retakeRequests) && $retakeRequests->count() > 0)
                ...
                @endphp
                <div class="mb-8 rounded-3xl border border-amber-200 bg-amber-50/70 p-6 shadow-sm">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-amber-200/80">
                        <div>
                            <h2 class="text-lg font-extrabold text-amber-950 flex items-center gap-2">
                                <span>📩 Permintaan Retake Kuis Kelas Ini</span>
                            ...
                            <p class="text-xs text-amber-800/80 mt-0.5">
                                Kelola pengajuan ujian ulang kuis dari mahasiswa di rombel kelas ini.
                            </p>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4 mb-3">
                        ...
                        <div class="relative w-full sm:w-72">
                            <input type="text" id="retake_search_input" placeholder="Cari nama atau email..."
                                   oninput="filterRetakeList()"
                        ...
                        <div class="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
                            <button type="button" onclick="setRetakeFilter('all')" id="rf_btn_all" class="retake-filter-btn px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-600 text-white transition">
                                Semua ({{ $retakeRequests->count() }})
                            </button>
                            <button type="button" onclick="setRetakeFilter('pending')" id="rf_btn_pending" class="retake-filter-btn px-2.5 py-1 text-xs font-bold rounded-lg bg-white text-amber-900 border border-amber-200 hover:bg-amber-100 transition">
                                Pending ({{ $pendingCount }})
                            </button>
                            <button type="button" onclick="setRetakeFilter('approved')" id="rf_btn_approved" class="retake-filter-btn px-2.5 py-1 text-xs font-bold rounded-lg bg-white text-emerald-800 border border-amber-200 hover:bg-emerald-50 transition">
                                Disetujui ({{ $approvedCount }})
                            </button>
                            <button type="button" onclick="setRetakeFilter('rejected')" id="rf_btn_rejected" class="retake-filter-btn px-2.5 py-1 text-xs font-bold rounded-lg bg-white text-rose-800 border border-amber-200 hover:bg-rose-50 transition">
                    ...
                    <div id="retake_list_container" class="space-y-2 max-h-[380px] overflow-y-auto pr-1" style="scrollbar-width: thin;">
                        @foreach($retakeRequests as $req)
                            <div class="retake-item-row flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-white border border-amber-200/80 rounded-2xl gap-3 transition hover:border-amber-300"
                                 data-name="{{ strtolower($req->user->name ?? '') }} {{ strtolower($req->user->email ?? '') }} {{ strtolower($req->quiz->title ?? '') }}"
                                 data-status="{{ $req->status }}">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-amber-100 text-amber-900 font-extrabold flex items-center justify-center text-xs flex-shrink-0">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center gap-1.5 text-xs text-slate-600 mt-0.5">
                                            ...
                                            <strong class="text-indigo-900 font-bold text-xs">{{ $req->quiz->title ?? 'Quiz' }}</strong>
                                            @if($req->reason)
                    ...
                    <div id="retake_empty_search_msg" class="hidden text-center py-6 text-xs text-amber-800/70 font-semibold">
                        Tidak ada pengajuan retake yang cocok dengan pencarian / filter.
                    </div>
                </div>

                <script>
                    let currentRetakeFilter = 'all';

                    function setRetakeFilter(status) {
                        currentRetakeFilter = status;
                        document.querySelectorAll('.retake-filter-btn').forEach(btn => {
                            btn.className = 'retake-filter-btn px-2.5 py-1 text-xs font-bold rounded-lg bg-white text-slate-700 border border-amber-200 hover:bg-amber-100 transition';
                        });
                        const activeBtn = document.getElementById('rf_btn_' + status);
                        if (activeBtn) {
                            activeBtn.className = 'retake-filter-btn px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-600 text-white transition';
                        }
                        filterRetakeList();
                    }

                    function filterRetakeList() {
                        const q = (document.getElementById('retake_search_input')?.value || '').toLowerCase().trim();
                        const rows = document.querySelectorAll('.retake-item-row');
                        let visibleCount = 0;
                        ...
                        rows.forEach(row => {
                            ...
                            const matchesStatus = currentRetakeFilter === 'all' || status === currentRetakeFilter;
                        ...
                        const emptyMsg = document.getElementById('retake_empty_search_msg');
                        if (emptyMsg) {
