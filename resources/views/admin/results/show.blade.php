        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="mb-8 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div>
                    ...
                    <p class="mt-2 text-slate-500">Detail hasil quiz mahasiswa, status verifikasi, dan jawaban attempt.</p>
                </div>
            ...
            <div class="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="border-b border-slate-200 px-6 py-5">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Rincian jawaban mahasiswa untuk attempt ini.</p>
                </div>
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 flex-wrap mb-2">
                                            ...
                                            @if(!is_null($answer->is_correct))
                                                @if($answer->is_correct)
                                                    <span class="inline-flex rounded-full border border-emerald-200 bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">Correct</span>
                                                @else
                                                    <span class="inline-flex rounded-full border border-rose-200 bg-rose-100 px-3 py-1 text-xs font-semibold text-rose-700">Incorrect</span>
                                                @endif
                                            @endif
                                        </div>
                                        ...
                                        @if($answer->question && $answer->question->question_type === 'multiple_choice')
                                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                                <div class="rounded-xl bg-slate-50 border border-slate-200 p-3">
                                                    ...
                                                    <p class="mt-1 font-semibold text-slate-900">{{ $answer->selected_option ?? '-' }}</p>
                                                </div>
                                                <div class="rounded-xl bg-slate-50 border border-slate-200 p-3">
                                                    <p class="text-slate-400">Correct Answer</p>
                                                    <p class="mt-1 font-semibold text-slate-900">{{ $answer->question->correct_answer ?? '-' }}</p>
                                                </div>
                                            </div>
                                        @else
                                            <div class="mt-4 rounded-xl bg-slate-50 border border-slate-200 p-4">
                                                <p class="text-sm text-slate-400 mb-2">Student Answer</p>
                                                <p class="text-slate-800 whitespace-pre-line">{{ $answer->answer_text ?? '-' }}</p>
                                            </div>
                                        @endif
                                        ...
                                        @if(!empty($answer->feedback))
                                            <div class="mt-4 rounded-xl bg-amber-50 border border-amber-200 p-4">
                                                ...
                                                <p class="text-sm text-amber-800">{{ $answer->feedback }}</p>
                                            </div>
                                        @endif
                                    </div>
                ...
                @else
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="mt-2 text-sm text-slate-500">Attempt ini belum memiliki data jawaban yang dapat ditampilkan.</p>
                    </div>
                @endif
            </div>
