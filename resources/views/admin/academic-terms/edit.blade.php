<x-app-layout>
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        ...
                        <h2 class="font-bold text-xl text-slate-900 leading-tight mt-0.5">
                            Edit Periode Semester: {{ $academicTerm->name }}
                        </h2>
                        <p class="text-xs text-slate-500 mt-0.5">
                            Perbarui informasi nama semester, tahun ajaran, dan tanggal aktif perkuliahan.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- FORM CARD -->
            <div class="bg-white border border-slate-200 shadow-xs rounded-2xl p-6">
                <form action="{{ route('admin.academic-terms.update', $academicTerm) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="term_type" class="w-full rounded-xl border-slate-300 focus:border-blue-600 focus:ring-blue-600 text-xs font-semibold" required>
                                ...
                                <option value="ganjil" {{ old('term_type', $academicTerm->term_type) == 'ganjil' ? 'selected' : '' }}>Ganjil</option>
                                <option value="genap" {{ old('term_type', $academicTerm->term_type) == 'genap' ? 'selected' : '' }}>Genap</option>
                            </select>
                            @error('term_type')
                                <p class="text-rose-600 text-xs mt-1">{{ $message }}</p>
                            @enderror
