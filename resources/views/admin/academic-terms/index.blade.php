<x-app-layout>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-xl text-gray-800 leading-tight">
                    Pengelolaan Matkul & Pembuatan Kelas Per-Semester
                </h2>
                <p class="text-sm text-gray-500">
                    Pusat operasional penetapan semester, pembukaan mata kuliah induk, pembuatan rombel kelas, dan penugasan Dosen Pengampu.
                </p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div class="space-y-1">
                        ...
                        <h3 class="text-lg font-bold text-gray-900">
                            {{ $selectedTerm->name ?? 'Pilih Semester' }} (Tahun Akademik: {{ $selectedTerm->academic_year ?? '-' }})
                        </h3>
                        <p class="text-xs text-gray-500">
                            Gunakan pemilih semester di sebelah kanan untuk melihat atau mengaktifkan semester berjalan.
                        </p>
                    </div>

                    <!-- SEMESTER SELECTOR DROPDOWN & CONTROL ACTIONS -->
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div class="relative">
                            <select onchange="window.location.href='?term_id=' + this.value" 
                                    class="w-full appearance-none bg-slate-50 border border-slate-300 focus:border-teal-600 focus:ring-teal-600 font-extrabold text-slate-800 text-xs py-2.5 pl-4 pr-10 rounded-xl shadow-xs cursor-pointer">
                        ...
                        <div class="flex items-center gap-2">
                            @if($selectedTerm && !$selectedTerm->is_active)
                                <form action="{{ route('admin.academic-terms.toggle-active', $selectedTerm->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-3 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition whitespace-nowrap">
                            ...
                            <button type="button" 
                                    @click="showEditTermModal = true"
                                    class="px-3.5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-xs rounded-xl transition flex items-center gap-1.5 whitespace-nowrap">
                                <span>✏️ Edit Tahun</span>
                            </button>

                            <button type="button" 
                                    @click="showCreateTermModal = true"
                                    class="px-3.5 py-2.5 bg-teal-50 hover:bg-teal-100 text-teal-700 border border-teal-200 font-bold text-xs rounded-xl transition flex items-center gap-1.5 whitespace-nowrap">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-1">
                    ...
                    <p class="text-[11px] text-teal-600">Pada {{ $selectedTerm->name ?? 'Semester' }}</p>
                </div>
            ...
            <!-- MASTER COURSE CATALOGUE & CLASS CREATION PORTAL -->
            <div class="space-y-6">
                <div class="flex items-center justify-between border-b border-gray-200 pb-3">
                    <h3 class="text-base font-extrabold text-gray-900 flex items-center gap-2">
                        <span>📚 Katalog Master Course & Pengelolaan Rombel Kelas</span>
                        <span class="text-xs font-bold text-gray-500">({{ $selectedTerm->name ?? '' }})</span>
                    </h3>
                </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    @foreach($allMasterCourses as $mc)
                        @php
                            $offerings = $groupedOfferings->get($mc->id, collect());
                            $isOpenedInTerm = $offerings->count() > 0;
                        @endphp

                        <div class="bg-white border {{ $isOpenedInTerm ? 'border-teal-200 shadow-sm' : 'border-gray-200' }} rounded-2xl overflow-hidden transition">
                            
                            <!-- Master Course Header Card -->
                            <div class="p-4 {{ $isOpenedInTerm ? 'bg-teal-50/60' : 'bg-slate-50/80' }} border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div class="space-y-1">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @if($isOpenedInTerm)
                                            <span class="text-xs font-extrabold text-emerald-800 bg-emerald-100 px-3 py-0.5 rounded-full border border-emerald-200">
                                                🟢 Dibuka di Semester Ini ({{ $offerings->count() }} Rombel)
                                            </span>
                                        @else
                                            <span class="text-xs font-bold text-gray-500 bg-gray-100 px-2.5 py-0.5 rounded-full border border-gray-200">
                                                ⚪ Belum Dibuka di Semester Ini
                                            </span>
                                        @endif
                                    </div>
                                ...
                                <div class="flex items-center gap-2 whitespace-nowrap">
                                    ...
                                    @else
                                        <button type="button" 
                                                @click="selectedMasterCourseId = '{{ $mc->id }}'; selectedMasterCourseName = '{{ addslashes($mc->name) }}'; showCreateOfferingModal = true"
                                                class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition flex items-center gap-1.5">
                                            <span>🚀 Buka Matkul Ini di {{ $selectedTerm->name ?? 'Semester' }}</span>
                                        </button>
                                    @endif

                                    <a href="{{ route('admin.master-courses.show', $mc->id) }}" 
                                       class="px-3 py-2 bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 font-bold text-xs rounded-xl transition">
                                        📄 Detail Matkul
                                    </a>
                                </div>
                            </div>

                            <!-- Opened Rombel Classes Table (If Opened) -->
                            @if($isOpenedInTerm)
        ...
        <!-- MODAL 1: BUKA ROMBEL KELAS BARU (CREATE OFFERING MODAL) -->
        <div x-show="showCreateOfferingModal" 
             ...
             style="display: none;">
            <div @click.away="showCreateOfferingModal = false" class="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-lg overflow-hidden">
                <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        <h3 class="text-base font-extrabold text-gray-900">🚀 Pembuatan Kelas & Penugasan Dosen</h3>
                        <p class="text-xs text-gray-500 mt-0.5">Tentukan Nama Rombel Kelas dan Dosen Pengampu pada semester {{ $selectedTerm->name ?? '' }}.</p>
                    </div>
                ...
                <form action="{{ route('admin.academic-terms.offerings.store') }}" method="POST" class="p-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1.5">Mata Kuliah Induk <span class="text-rose-500">*</span></label>
                        <select name="master_course_id" x-model="selectedMasterCourseId" class="w-full rounded-xl border-gray-300 focus:border-teal-600 focus:ring-teal-600 text-xs font-bold" required>
                            <option value="">-- Pilih Master Course --</option>
                            @foreach($allMasterCourses as $mc)
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1.5">Semester Akademik Destinasi <span class="text-rose-500">*</span></label>
                        <select name="academic_term_id" class="w-full rounded-xl border-gray-300 focus:border-teal-600 focus:ring-teal-600 text-xs font-bold bg-gray-50" required>
                            @foreach($terms as $term)
                                <option value="{{ $term->id }}" @selected($selectedTerm?->id === $term->id)>
                                    {{ $term->is_active ? '🟢 [AKTIF]' : '⚪' }} {{ $term->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">Threshold Sertifikat (%) <span class="text-rose-500">*</span></label>
                    ...
                    <div class="pt-3 border-t border-gray-100 flex items-center justify-end gap-2">
                        ...
                        <button type="submit" class="px-5 py-2 bg-teal-600 hover:bg-teal-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition">🚀 Buka Rombel Kelas</button>
                    </div>
                </form>
            </div>
        </div>

        <!-- MODAL 2: EDIT ROMBEL KELAS (EDIT OFFERING MODAL) -->
        ...
        <!-- MODAL 3: EDIT TAHUN AKADEMIK SEMESTER (EDIT TERM MODAL) -->
        @if($selectedTerm)
        <div x-show="showEditTermModal" 
            ...
            <div @click.away="showEditTermModal = false" class="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md overflow-hidden">
                ...
                <form action="{{ route('admin.academic-terms.update', $selectedTerm->id) }}" method="POST" class="p-5 space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1.5">Nama Semester <span class="text-rose-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name', $selectedTerm->name) }}" class="w-full rounded-xl border-gray-300 focus:border-teal-600 focus:ring-teal-600 text-xs font-bold" required>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">Tahun Akademik <span class="text-rose-500">*</span></label>
                            <input type="text" name="academic_year" value="{{ old('academic_year', $selectedTerm->academic_year ?? '2026/2027') }}" placeholder="Contoh: 2026/2027" class="w-full rounded-xl border-gray-300 focus:border-teal-600 focus:ring-teal-600 text-xs font-bold" required>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="term_type" class="w-full rounded-xl border-gray-300 focus:border-teal-600 focus:ring-teal-600 text-xs font-bold" required>
                                <option value="ganjil" @selected(($selectedTerm->term_type ?? '') === 'ganjil')>Ganjil</option>
                                <option value="genap" @selected(($selectedTerm->term_type ?? '') === 'genap')>Genap</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="pt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" @checked($selectedTerm->is_active) class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                    ...
                    <div class="pt-3 border-t border-gray-100 flex items-center justify-end gap-2">
                        ...
                        <button type="submit" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition">💾 Simpan Tahun Akademik</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <!-- MODAL 4: TAMBAH PERIODE SEMESTER BARU (CREATE TERM MODAL) -->
        <div x-show="showCreateTermModal" 
             x-transition.opacity
             class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4"
             style="display: none;">
            <div @click.away="showCreateTermModal = false" class="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md overflow-hidden">
                <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                    <div>
                    ...
                    <button type="button" @click="showCreateTermModal = false" class="text-gray-400 hover:text-gray-600 text-lg font-bold">✕</button>
                </div>
                ...
                <form action="{{ route('admin.academic-terms.store') }}" method="POST" class="p-5 space-y-4">
                    ...
                    <div class="pt-3 border-t border-gray-100 flex items-center justify-end gap-2">
                        <button type="button" @click="showCreateTermModal = false" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-xs rounded-xl transition">Batal</button>
