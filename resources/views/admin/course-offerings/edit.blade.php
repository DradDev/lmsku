<x-app-layout>
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        ...
                        <h2 class="font-bold text-xl text-slate-900 leading-tight mt-0.5">
                            Edit Rombel: {{ $courseOffering->section_name }} ({{ $courseOffering->masterCourse->name ?? 'Course' }})
                        </h2>
                        <p class="text-xs text-slate-500 mt-0.5">
                            Perbarui rombel kelas, dosen pengampu, kuota mahasiswa, atau status perkuliahan.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- FORM CARD -->
            <div class="bg-white border border-slate-200 shadow-xs rounded-2xl p-6">
                <form action="{{ route('admin.course-offerings.update', $courseOffering) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full rounded-xl border-slate-300 focus:border-blue-600 focus:ring-blue-600 text-xs font-semibold" required>
                            <option value="draft" {{ old('status', $courseOffering->status) == 'draft' ? 'selected' : '' }}>Draft (Tertutup)</option>
                            <option value="published" {{ old('status', $courseOffering->status) == 'published' ? 'selected' : '' }}>Published (Terbuka)</option>
                            <option value="ongoing" {{ old('status', $courseOffering->status) == 'ongoing' ? 'selected' : '' }}>Ongoing (Sedang Berjalan)</option>
                            <option value="expired" {{ old('status', $courseOffering->status) == 'expired' ? 'selected' : '' }}>Expired (Kedaluwarsa)</option>
                            <option value="cancelled" {{ old('status', $courseOffering->status) == 'cancelled' ? 'selected' : '' }}>Cancelled (Dibatalkan)</option>
                        </select>
                        @error('status')
                            <p class="text-rose-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
